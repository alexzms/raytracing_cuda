...
namespace rt_cuda {
    namespace kernel_funcs {
        __global__ static void render_gradient_pic(uchar3* ptr, size_t size, size_t row_size, size_t column_size, unsigned char z) {
            ...
            ptr[offset].z = static_cast<unsigned char>((static_cast<float>(z) * vec3<float>{1, 2, 0.5f}).z());
        }
    }
    ...
    public:
        ...
        void run(unsigned char z = 51) const {
            ...
            if (_width % thread_x != 0 || _height % thread_y != 0)
                std::cout << "[output_an_image][warn] _row_size % thread_x != 0 || _column_size % thread_y != 0"
                          << std::endl;
            size_t block_x = _width / thread_x, block_y = _height / thread_y;
            dim3 grid(block_x, block_y);
            dim3 block(thread_x, thread_y);
            kernel_funcs::render_gradient_pic<<<grid, block>>>
                                                (_d_ptr, _ptr_size, _width, _height, z);
    ...
    private:
        ...
        static void decompose_number(T k, T *a, T *b) {
            ...
            while (sqrt_k != 0) {                                       // we don't care too much performance issue here
                if (k % sqrt_k ==
                    0)                                    // because this function will only be called once
                {
                    *a = sqrt_k;
                    *b = k / sqrt_k;
                    return;
            ...
            *a = 0;
            *b = 0;                                             // just to make clang-tidy happy...
        }
